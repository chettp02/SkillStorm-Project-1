...
class AddConfirmation extends React.Component {
  ...
  onSubmit() {
    ...
    console.log(this.state.passengerCount);
    console.log(this.state.passengerLimit);
    if (this.state.passengerCount >= this.state.passengerLimit) {
      ...
      APIService.createConfirmation(confirmationObject)
        .then((result) => {
          console.log(result.data);
          window.alert("Your submit was successful!");
        })
        .catch((error) => {
  ...
  render() {
    return (
      <div>
        <h1 className="text-center my-5">Create New Confirmation</h1>
        <p className="mx-3">Select the Passenger</p>
